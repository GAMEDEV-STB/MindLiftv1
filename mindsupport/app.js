...
function renderMoodChart() {
    ...
    if (!moods.length) {
        wrap.innerHTML = '<p style="color:var(--text-dim);font-size:.85rem;margin:auto">No mood data yet. Tap an emoji above!</p>';
        return;
    }
    wrap.innerHTML = moods.map(m => {
        ...
        return `<div class="mood-bar" style="height:${h}px" title="${m.label} — ${d.toLocaleString()}"><span class="mood-bar-label">${dayLabel}</span></div>`;
    }).join('');
}
